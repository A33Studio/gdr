...
export default async function Project({ params }: {
    ...
    {p.media[0] && <p className="cover-caption">{p.media[0].caption} <a href={p.media[0].src} target="_blank" rel="noreferrer">View full size ↗</a></p>}
    ...
    {p.chapters.map((chapter, i) => <section id={chapter.id} className="case-section reveal" key={chapter.id}><p className="eyebrow">{String(i + 1).padStart(2, '0')} / The process</p><h2>{chapter.title}</h2>{chapter.paragraphs.map(text => <p key={text}>{text}</p>)}{chapter.points && <ul>{chapter.points.map(text => <li key={text}>{text}</li>)}</ul>}{p.media.filter(m => m.section === chapter.id).map(m => <figure className="inline-project-figure" key={m.src}><a href={m.src} target="_blank" rel="noreferrer" aria-label={`Open full-size image: ${m.alt}`}><img src={m.src} alt={m.alt} width={m.width} height={m.height} loading="lazy"/></a><figcaption>{m.caption} <a href={m.src} target="_blank" rel="noreferrer">View full size ↗</a></figcaption></figure>)}</section>)}
