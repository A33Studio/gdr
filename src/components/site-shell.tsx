...
export function SiteHeader() {
  ...
  return <header className="site-header"><div className="header-inner wrap">
    ...
    <button type="button" className="theme-toggle" data-dark={dark} onClick={()=>setTheme(dark?'light':'dark')} aria-label={dark?SITE.navigation.themeLight:SITE.navigation.themeDark} title={dark?SITE.navigation.themeLight:SITE.navigation.themeDark}><span className="theme-thumb" aria-hidden="true"/><Sun size={15} aria-hidden="true"/><Moon size={15} aria-hidden="true"/></button>
  </div></header>;
}
