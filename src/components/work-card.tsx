...
export function ProjectCover({ project, index = 0, large = false }: {
    ...
    {project.media[0] ? <img className="project-cover-photo" src={project.media[0].src} alt={project.media[0].alt}/> : project.slug === 'purdue-space-program' ? <div><div className="cover-title">Tadpole</div><div className="cover-flow"><span>250+<small>mount candidates</small></span><ArrowRight aria-hidden="true" size={23}/><span>3<small>configurations</small></span><ArrowRight aria-hidden="true" size={23}/><span>1<small>selected design</small></span></div></div> : <div className={project.coverUnit ? 'cover-metric' : 'cover-title'}>{project.coverValue}{project.coverUnit && <> <small>{project.coverUnit}</small></>}</div>}
