...
export function ProjectSection({project:p}:{project:CaseStudy}){
 ...
 return <article id={p.slug} className="project-section" data-nav-section>
  ...
  <div className="project-opening"><div className="project-summary"><dl className="case-meta"><div><dt>{L.role}</dt><dd>{p.role}</dd></div><div><dt>{L.timeline}</dt><dd>{p.dates}</dd></div><div><dt>{L.status}</dt><dd>{p.status}</dd></div></dl><p className="overview">{p.overview}</p><div className="work-tags">{p.tags.map(t=><span key={t}>{t}</span>)}</div>{p.links.length>0&&<div className="project-links">{p.links.map(l=><a href={l.href} key={l.href} className="text-link" target="_blank" rel="noreferrer">{l.label}<ArrowUpRight size={15}/></a>)}</div>}</div><figure className="opening-figure"><ProjectCover project={p}/>{p.media[0]&&<figcaption>{p.media[0].caption} <a href={p.media[0].src} target="_blank" rel="noreferrer">{L.fullImage} ↗</a></figcaption>}</figure></div>
  ...
  <section className="outcome"><p className="eyebrow">{L.outcome}</p><p>{p.outcome}</p><span className="status-label">{p.status}</span></section></div><a className="back-index text-link" href="#work"><ArrowUp size={15}/>{SITE.work.backToIndex}</a>
 </article>;
}
