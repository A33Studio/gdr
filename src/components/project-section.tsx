...
export function ProjectSection({project:p,index}:{project:CaseStudy;index:number}){
 ...
 return <article id={p.slug} className="project-section" data-nav-section>
  <header className="project-heading"><p className="eyebrow">{p.category}<span className="project-number">{String(index+1).padStart(2,'0')}</span></p><h2>{p.title}</h2><p className="project-deck">{p.description}</p></header>
